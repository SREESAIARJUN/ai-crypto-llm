...
def generate_historical_data():
    """Generate some historical data by triggering trades and market data updates"""
    print("\n🔍 Generating historical data...")
    
    # First, get current market data to update price history
    print("Getting market data...")
    market_response = requests.get(f"{BACKEND_URL}/market/data")
    if market_response.status_code == 200:
        print("✅ Market data fetched successfully")
    else:
        print(f"❌ Failed to fetch market data: {market_response.status_code}")
    
    # Trigger a trade to create trade markers
    print("Triggering a trade...")
    trade_response = requests.post(f"{BACKEND_URL}/trade/trigger")
    if trade_response.status_code == 200:
        print("✅ Trade triggered successfully")
    else:
        print(f"❌ Failed to trigger trade: {trade_response.status_code}")
    
    # Wait a bit to ensure data is processed
    time.sleep(2)
    
    # Trigger another market data update
    print("Getting market data again...")
    market_response = requests.get(f"{BACKEND_URL}/market/data")
    if market_response.status_code == 200:
        print("✅ Market data fetched successfully")
    else:
    ...
    print("Triggering another trade...")
    trade_response = requests.post(f"{BACKEND_URL}/trade/trigger")
    if trade_response.status_code == 200:
        print("✅ Trade triggered successfully")
    else:
...
def test_chart_data_timeframes():
    """Test Chart Data Endpoint with different timeframes after generating data"""
    print_separator()
